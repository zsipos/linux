...
static unsigned int picotcp_poll(struct file *file, struct socket *sock, poll_table *wait)
{
	...
	if (is_udp(psk)) {
		psk_stack_lock(psk);
		if (rem_pico_socket_udp_poll(psk->stack_chan, psk->pico) > 0) {
...
static void pico_event_clear(struct picotcp_sock *psk, uint16_t events)
{
	...
	if (is_udp(psk) && (events & PICO_SOCK_EV_RD)) {
		if (psk->udpcnt)
			psk->udpcnt--;
		if (psk->udpcnt < 0)
			printk("oops: pico_event_clear(): udcnt < 0\n");
		if (psk->udpcnt)
...
static int picotcp_connect(struct socket *sock, struct sockaddr *_saddr, int socklen, int flags)
{
	...
	if (ev & PICO_SOCK_EV_CONN) {
		...
		psk_stack_lock(psk);
#ifdef MINLOCK
		rem_stack_lock(psk->stack_chan);
#endif
		rem_pico_socket_close(psk->stack_chan, psk->pico);
		psk->in_use = 0;
#ifdef MINLOCK
		rem_stack_unlock(psk->stack_chan);
#endif
		psk_stack_unlock(psk);
		ret = -EINTR;
	}
...
static int picotcp_recvmsg_stream(struct socket *sock, struct msghdr *msg, size_t len, int flags)
{
	...
	while (tot_len < len) {
		...
		psk_stack_lock(psk);
		//psk_events_lock(psk);
		r = rem_pico_socket_recvfrom(psk->stack_chan, psk->pico, kbuf + tot_len, len - tot_len, &addr, &port);
		//psk_events_unlock(psk);
		psk_stack_unlock(psk);
		if (r < 0) {
...
static int picotcp_create(struct net *net, struct socket *sock, int protocol, int kern)
{
	...
	pico_stack_lock(stack_mutex, stack_chan);
#ifdef MINLOCK
	rem_stack_lock(stack_chan);
#endif
...
#ifdef MINLOCK
	rem_stack_unlock(stack_chan);
#endif
	pico_stack_unlock(stack_mutex, stack_chan);
